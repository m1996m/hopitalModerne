...
@Component({
  selector: 'app-personnel-edite',
  templateUrl: './personnel-edite.component.html',
  styleUrls: ['./personnel-edite.component.css']
})
export class PersonnelEditeComponent {
  ...
  showModal1(data: PersonnelEntity): void {
    this.isVisible = true;
    this.personnel = data;
    this.initForm();
  ...
  getHopital(){
    this.hopitalServie.recherche({"content":this.connexionService.userInfo?.pays}).subscribe((data:any)=>{
      this.hopitals =data;
    });
  }
  ...
  initForm(){
    this.formulaire = this.fb.group({
      ...
      hopital: [this.personnel?.hopital],
      date_naissance: [this.personnel?.date_naissance.toLocaleString().substring(0,10)],
  ...
  enregistrer(){
    this.personnelService.updatepersonnel(this.formulaire.value,this.personnel.slug).subscribe((data)=>{
      this.handleCancel();
    });
  }
